...
// Map an APPS1 raw count to the equivalent APPS2 raw that reads the SAME pedal
// %, so the scripted steps stay PLAUSIBLE (the dual-APPS T11.8 check must not
// false-trip on the BPPC/slew/fault steps). Implausibility is tested separately.
static inline uint16_t apps2FromApps1(uint16_t a1) {
    long pct = (a1 - VcuCal::APPS1_RELEASED_ADC) * 100 /
               (VcuCal::APPS1_PRESSED_ADC - VcuCal::APPS1_RELEASED_ADC);
    if (pct < 0)   pct = 0;
    if (pct > 100) pct = 100;
    return (uint16_t)(VcuCal::APPS2_RELEASED_ADC +
        pct * (VcuCal::APPS2_PRESSED_ADC - VcuCal::APPS2_RELEASED_ADC) / 100);
}
...
inline void vcuSelfTest(VcuApp &app, VcuTestSink sink) {
    ...
    for (int i = 0; i < VCU_TEST_STEPS; i++) {
        ...
        // The script's apps/bps are abstract 0..1023 (fraction of full travel);
        // scale to the actual calibrated endpoints so the test is resolution-
        // independent (works whether the build is 10- or 12-bit).
        uint16_t a1 = (uint16_t)((uint32_t)s.apps * VcuCal::APPS1_PRESSED_ADC / 1023);
        uint16_t bp = (uint16_t)((uint32_t)s.bps  * VcuCal::BPS_PRESSED_ADC   / 1023);
        app.sample(a1, apps2FromApps1(a1), bp);
