...
// ---------------- Pins / rates ----------------
// VCU PCB (confirmed 2026-08-29): APPS1=A8, APPS2=A7, BPS=A6.
// This first real-board trial still uses SINGLE APPS (A8) with the existing
// 10-bit VcuApp logic, to confirm the firmware runs on the actual hardware.
// APPS2 (A7) dual-sensor T11.8 plausibility + measured calibration are the
// next step, not this trial.
#define APPS_PIN  A8         // APPS1 (5 V sensor via x0.662 divider)
#define APPS2_PIN A7         // APPS2 (3.3 V sensor via x0.675 divider)
#define BPS_PIN   A6         // BPS
...
static void controlTask(void *) {
    ...
    for (;;) {
        // --- 1 kHz: filtering only (dual APPS + brake) ---
        uint16_t rawA1 = analogRead(APPS_PIN);    // APPS1 (A8)
        uint16_t rawA2 = analogRead(APPS2_PIN);   // APPS2 (A7)
        uint16_t rawB  = analogRead(BPS_PIN);     // BPS   (A6)
        app.sample(rawA1, rawA2, rawB);
        scopeSample(rawA1, app.appsFiltered(), rawB, app.bpsFiltered());
...
static void telemetryTask(void *) {
    ...
    for (;;) {
        ...
        if (have)
            Serial.printf("[mini] A1 %u(%.0f%%) A2 %u(%.0f%%) %s BPS %u(%.0f%%) BPPC %s "
                          "stale %d inv %d | trq %.1f Vin %.1f Flt %u | ctrl %lu tick %lu millis %lu\n",
                s.out.appsADC, s.out.apps1Pct, s.out.apps2ADC, s.out.apps2Pct,
                s.out.appsImplaus ? "IMPL" : "ok  ",
                s.out.bpsADC, s.out.brakePct,
...
void setup() {
    ...
    analogReadResolution(12);        // 12-bit (0..4095) to match the calibration
